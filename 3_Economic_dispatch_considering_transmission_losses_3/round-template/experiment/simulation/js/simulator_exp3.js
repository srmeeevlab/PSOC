...
function Simulate() {
    ...
    if(!pd || isNaN(pd)){
        alert("pd value given is incorrect. setting to 100")
    ...
    let opt_tab=''
    let ifc_tab=''
    for (let index = 0; index < opt_gen[0].length; index++) {
        opt_tab+= `<tr ><td>${opt_gen[0][index]}</td><td>${opt_gen[1][index].toFixed(4)}</td></tr>`
    }
    for (let index = 0; index < ifc_pf[0].length; index++) {
        ifc_tab+= `<tr ><td>${ifc_pf[0][index]}</td><td>${ifc_pf[1][index].toFixed(4)}</td><td>${ifc_pf[2][index].toFixed(4)}</td></tr>`
    }
    ...
    <table>
        <thead>
            <th>Unit No</th>
            <th>Optimal Generation (in MW)</th>
        </thead>
        ${opt_tab}
    </table>
    `
